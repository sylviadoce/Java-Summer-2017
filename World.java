...
public class World {
	...
	/**
	 * Constructs a world that is by default 80 x 60 cells, with a cell size of 10.
	 */
	public World() {
		this(80, 60, 10);
	}
	...
	/**
	 * Creates a world from the file description using the given scale for cell size.
	 * @param path - path to the world file
	 * @param scale - the size of each cell
	 */
		...
		if (input.length() > 0) {
			...
			for (int x = 0 ; x < width ; x++) {
				for (int y = 0 ; y < height ; y++) {
					...
					case '%': grid[x][y].setType(WALL); break;
					case 'P': 
	...
	/**
	 * Returns the cell at the given x, y position, or null if no cell is at that position.
